...
- **Always** use the `compose.yml` file to run the PostgreSQL database for local development and testing
- Start the database with `docker compose up -d` or `docker compose up` (for logs)
- Stop the database with `docker compose down`
- The database runs on port 5432 with trust authentication (no password required for local dev)
